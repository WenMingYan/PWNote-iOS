...
@interface TYDFTimerPickerView()<TYDFTimerPickerColumnViewDelegate>
...
@property (nonatomic, strong) NSArray<TYDFTimerPickerColumnView *> *columnViewList;

@property (nonatomic, assign) BOOL isSubViewLayouted;
@property (nonatomic, assign) BOOL isSelected;
@end

@implementation TYDFTimerPickerView
...
- (TYDFTimerPickerColumnView *)createColumnViewAtComponent:(NSUInteger)component refresh:(BOOL)refresh {
    ...
    TYDFTimerPickerColumnView *view = [self columnViewInComponent:component];
    if (!view) {
        ...
        view = [[TYDFTimerPickerColumnView alloc]initWithFrame:CGRectMake(component * width, 0, width, self.height) rowHeight: rowHeight upLineHeight:upLineHeight downLineHeight:downLineHeight isCycleScroll:self.isCycleScroll datas:datas];
        view.delegate = self;
...
- (void)setupColumnView {
    ...
    for (int i = 0; i < _numberOfComponents; i++) {
        TYDFTimerPickerColumnView *view = [self createColumnViewAtComponent:i refresh:false];
...
- (TYDFTimerPickerColumnView *)columnViewInComponent:(NSUInteger)component {
    if (!self.columnViewList || self.columnViewList.count == 0) {
        return nil;
    }
    __block TYDFTimerPickerColumnView *view = nil;
    [self.columnViewList enumerateObjectsUsingBlock:^(TYDFTimerPickerColumnView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
